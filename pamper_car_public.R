...
##
## Objective: to assess markers of inflammation and endothelial damage in PAMPer patients
## 
## CAR: Clustering and Regression
##
## Project Support: NIH T32 in Trauma and Sepsis, University of Pittsburgh Department of Surgery
##
## Written by Danielle Gruen, April 2019
...
## Figure: Zoomed in boxplot
## keeps outliers but zooms in with facet wrap
## issue described here: https://stackoverflow.com/questions/25124895/no-outliers-in-ggplot-boxplot-with-facet-wrap
    ...
    tab1 <- CreateTableOne(vars = myVars, strata = "alive_at_30", data = df, testNonNormal = kruskal.test)
    tab1 <- print(tab1, nonnormal = TRUE, quote = FALSE, noSpaces = TRUE, printToggle = FALSE, missing=TRUE)
    View(tab1)
...
## Model 
    for (i in 1:length(term)){
      ...
      glm.results <- glm(Concentration ~ FFP + age + iss + PH_blood + PH_intubation + gender +
                           PH_crystalloid + PH_sbp_70 + ais_head + MOI + total_gcs_score + transfer +
                           prbc_24h + plasma_24h + platelets_24h + crystalloid_24h +
                           INR + ed_lowest_sbp
                         ,
                         data = df.i)
...
#PCA
    ...
    ## plot contribution of variables to first # dimensions
...
## Assessment
    ...
    # add a column to compare only the two main clusters
    # that groups clusters 2 and 3 into 1
    
    df = PAMPer_clustered
    
    df$cluster <- NA # make a new column to group cluster 1&2 and 3&4
    
    # df$cluster[which(df$groups == "1")] = 1 # cluster 1 or 2
    # df$cluster[which(df$groups == "2")] = 1 # cluster 1 or 2
    # df$cluster[which(df$groups == "3")] = 2 # cluster 2 or 3
    # df$cluster[which(df$groups == "4")] = 2 # cluster 2 or 3

    df$cluster <- df$groups
    
    df$cluster <- as.factor(df$cluster) # make it a factor
    
    PAMPer_clustered = df
    
    ## merge cluster information with clinical data
    PAMPer_all_data_clustered = merge(df_master_filtered_wide, PAMPer_clustered, by="stnum")
    
    df = PAMPer_all_data_clustered
    tally_clusters <- df %>%
      group_by(cluster) %>%   # group by Biomarker
      tally()
    tally_clusters
